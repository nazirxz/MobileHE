...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = useCallback((username: string, password: string) => {
    ...
    if (patient) {
      ...
      setUserRole("pasien");
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ userId: patient.id, role: "pasien" }));
    ...
    if (nurse) {
      ...
      setUserRole("perawat");
      localStorage.setItem(STORAGE_KEY, JSON.stringify({ userId: nurse.id, role: "perawat" }));
  ...
  // Merge base sessions with overrides, applying default approvalStatus
  const getPatientSessions = useCallback((patientId: string): SessionRecord[] => {
    const patient = patients.find((p) => p.id === patientId);
    const base = (patient?.sessions ?? []).map((s) => ({
      ...s,
      // Mock data sessions with no approvalStatus default to 'disetujui' (pre-approved)
  ...
  const completeSession = useCallback((patientId: string, record: SessionRecord) => {
    const recordWithApproval: SessionRecord = {
      ...record,
      approvalStatus: "menunggu", // Always set to pending when newly completed
    };
  ...
  const approveSession = useCallback((patientId: string, day: number, status: "disetujui" | "ditolak", note?: string) => {
    setSessionOverrides((prev) => {
      ...
      const baseSessions = (basePatient?.sessions ?? []).map((s) => ({
        ...s,
        approvalStatus: s.approvalStatus ?? (s.status === "selesai" ? "disetujui" : undefined),
      }));
      // Merge to find the session
